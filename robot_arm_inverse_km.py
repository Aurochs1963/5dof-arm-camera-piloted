...
if __name__ == "__main__":
    ...
    while True:
        ...
        if lect == "o":
            ...
            # Récupération du dictionnaire des objets
            objects_dict = data.get("objects", {})
            
            # 2. Listage des objets disponibles
            print("--- Objets trouvés dans le fichier ---")
            if not objects_dict:
                print("Aucun objet trouvé.")
                choix = "s"
            else:
                for obj_id, values in objects_dict.items():
                    print(f"ID: {obj_id} -> Données: {values}")
                
                # Le tag "531" correspondant au pot
                try:
                    selected_obj = objects_dict["531"]
                    xpot = float(selected_obj.get("x", 0.0))
                    ypot = float(selected_obj.get("y", 0.0))
                    zpot = float(selected_obj.get("z", 0.0))
                    #On sauvegarde la position du pot car ensuite l'ID 531 peut etre caché par un objet dans le pot
                    xpot_save = xpot
                    ypot_save = ypot
                    zpot_save = zpot
                except KeyError as e:
                    #On restaure la dernière position connue du pot
                    xpot=xpot_save
                    ypot=ypot_save
                    zpot=zpot_save
                    
                print("Position du pot (tag #531): xpot=",xpot,"ypot=",ypot,"zpot=",zpot)

                # 3. Demander à l'utilisateur de choisir
                print("-" * 30)
                choice = input("Entrez l'ID de l'objet que vous souhaitez choisir (ex: 90) : ").strip()

                # 4. Vérification et remplissage des variables
                if choice in objects_dict:
                    selected_obj = objects_dict[choice]

                    # Remplissage des variables demandées
                    # Note : 'frame' est à la racine du JSON, pas dans l'objet
                    frame = int(data.get("frame", 0)) 
                    
                    x = float(selected_obj.get("x", 0.0))
                    y = float(selected_obj.get("y", 0.0))
                    z = float(selected_obj.get("z", 0.0))
                    
                    xmoy = float(selected_obj.get("xmoy", 0.0))
                    ymoy = float(selected_obj.get("ymoy", 0.0))
                    zmoy = float(selected_obj.get("zmoy", 0.0))

                    # 5. Affichage pour confirmation
                    print("\nObjet choisi :",choice,"frame:",frame,"x:",x,"y:",y,"z:",z,"xmoy:",xmoy,"ymoy",ymoy,"zmoy",zmoy)
                    #choix = input("valeur cam réelle, cam moyenne ou saisie (r/m/s) ? :")
                    choix = "r"
                else:
                    print(f"Erreur : L'ID '{choice}' n'existe pas dans la liste.")
                    choix = "s"
        ...
        if choix == "s":
             x,y,z = map(float,input("Input positions values x y z (in milimeters): ").split())
        ...
        #Approche
        target = ((x/1000)+dx,(y/1000)+dy,(z/1000)+(2*dz))
        move_bras(target,"1000",speed,delay,"n")   
